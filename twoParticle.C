...
void twoParticle(char * tag= "0", char *infName = "/d102/yjlee/hiForest2MC/Pythia80_HydjetDrum_mix01_HiForest2_v22_simTrack05.root")
{
...
//      if (nTrk<180) continue;
      ...
      for (int i2=i+1;i2<i+c2->GetEntries();i2++) {
         ...
         if (i!=(i2%c->GetEntries())&&fabs(c2->evt.vz-c->evt.vz)<1&&fabs(c2->track.nTrk-c->track.nTrk)<200) {
            selected = i2;
            break;
         }
      }
      cout <<i<<" "<<selected<<" "<<c->track.nTrk<<" "<<c2->track.nTrk<<" "<<c->evt.vz<<" "<<c2->evt.vz<<" "<<nTrk<<endl;


      for (int j=0;j<c->track.nTrk;j++) {
         if (fabs(c->track.trkEta[j])>2.4) continue;
//         if (!c->selectTrack(j)) continue;
	 if (fabs(c->track.trkPt[j])<1||fabs(c->track.trkPt[j])>4) continue;
         for (int j2=j+1;j2<c->track.nTrk;j2++) {
            if (fabs(c->track.trkEta[j2])>2.4) continue;
//            if (!c->selectTrack(j2)) continue;
   	    if (fabs(c->track.trkPt[j2])<1||fabs(c->track.trkPt[j2])>4) continue;
...
//            if (!c->selectTrack(j2)) continue;
   	    if (fabs(c2->track.trkPt[j2])<1||fabs(c2->track.trkPt[j2])>4) continue;
